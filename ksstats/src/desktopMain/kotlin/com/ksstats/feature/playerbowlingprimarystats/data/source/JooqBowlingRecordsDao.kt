...
class JooqBowlingRecordsDao(private val databaseConnections: DatabaseConnections) : BowlingRecordsDao {
    ...
    override fun getMatchTotals(searchParameters: SearchParameters): Flow<DatabaseResult<InningsByInningsBowling>> =
        flow {
            ...
            DriverManager.getConnection(
                ...
                val count = countResult[0].getValue("count", Int::class.java)
    ...
    override fun getGroundAverages(searchParameters: SearchParameters): Flow<DatabaseResult<PrimaryBowling>>  = flow {
        val databaseConnection = databaseConnections.connections[searchParameters.matchType.value]
            ?: throw Exception("Database connection for match type ${searchParameters.matchType} not found")
        ...
        DriverManager.getConnection(
            ...
            val cteStep10Name = "bocrto_bbm"
            val bocrto_tmp_bowl =
                JooqBowlingGroundRecords.createTemporaryBowlingCte(searchParameters)
            val bocrto_tmp_teams = JooqBowlingCareerRecords.createTemporaryTeamsCte(searchParameters)

            val bocrto_results = JooqBowlingGroundRecords.createResultsCte(
                searchParameters,
                cteStep1Name,
                cteStep2Name,
                cteStep5Name,
            ...
            val bocrto_count = JooqBowlingGroundRecords.totalCountsCte(cteStep3Name)

            val bocrto_fives = JooqBowlingGroundRecords.createFives(cteStep1Name, searchParameters.fivesLimit)
            val bocrto_tens = JooqBowlingGroundRecords.createTens(cteStep1Name)
            val bocrto_bbm_all = JooqBowlingGroundRecords.createBbmAll(cteStep1Name)
            val bocrto_bbm = JooqBowlingGroundRecords.createBbm(cteStep9Name)
